...
import type { CreateTrainingCommand } from "../../../application/training/commands/CreateTrainingCommand";
import { createTrainingHandler } from "../../../application/training/handlers/createTrainingHandler";
import { listTrainingsHandler } from "../../../application/training/handlers/listTrainingsHandler";
...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const trainings = await listTrainingsHandler(unitOfWork, { teamId });
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const payload = (await request.json()) as CreateTrainingCommand;
    const created = await createTrainingHandler(unitOfWork, payload);
